...
if [ ! -r "$OSM_NOICE_BASE.db" ] && [ ! -r "$OSM_NOICE_BASE.shp" ] && [ ! -r "$OSM_NOICE_BASE.dump" ] ; then
	rm -f "$OSM_NOICE_BASE.shp" "$OSM_NOICE_BASE.shx" "$OSM_NOICE_BASE.dbf" "$OSM_NOICE_BASE.prj" "$OSM_NOICE_BASE.cpg" "$OSM_NOICE_BASE.db" "$OSM_NOICE_BASE.dump"
	if [ -z "$OSMIUM_NOICE" ] ; then

		osmium tags-filter --verbose --overwrite --output=$OSM_NOICE_BASE.filter.osm.pbf \
			--remove-tags "$OSM_NOICE" \
			a/natural!=bay,cliff,sinkhole,cave_entrance,crevasse,dune,desert,valley,volcano,coastline;

		osmium export --verbose --config=`dirname $0`/osmium-export-config.json \
			--geometry-types=polygon --output-format=pg --overwrite --output=$OSM_NOICE_BASE.dump \
			$OSM_NOICE_BASE.filter.osm.pbf
...
if [ -r "$OSM_NOICE_BASE.dump" ] ; then
	OSM_NOICE_SOURCE="$OSM_NOICE_BASE.dump"
elif [ -r "$OSM_NOICE_BASE.db" ] ; then
